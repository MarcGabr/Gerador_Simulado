...
public class EditarSemMultiplaEscolha extends AbstractCadastroQuestoes{
    ...
    public EditarSemMultiplaEscolha(ArrayList<Questao> questao, ActionListener menu, Assunto assunto,int indexQuestao) {
        super(questao, menu, assunto);
        QuestaoMultiplaEscolhaSem o=(QuestaoMultiplaEscolhaSem) questao.get(indexQuestao);
        this.indexQuestao=indexQuestao;
        this.QuantidadeAlternativa=o.getAlternativa().length;
        this.pergunta.setText(o.getPergunta());
    ...
    void newButtons(JPanel panel, ArrayList<Questao> questao) {
        ...
        editar.addActionListener((ActionEvent ae) -> {
                if (isValido()) {
                    if (questao.set(indexQuestao,new QuestaoMultiplaEscolhaSem(pergunta.getText(), respostaQuestao, alternativasQuestao, assunto))!=null) {
                        dispose();
                    } else {
                        JOptionPane.showMessageDialog(this,"ERRO","Questao já cadastrada!", JOptionPane.WARNING_MESSAGE);
                    }
                } else {
                    JOptionPane.showMessageDialog(this,"ERRO","Impossivel cadastrar\nDigite TODOS os dados corretamente",JOptionPane.WARNING_MESSAGE);
                }
        });
    }
    ...
    @Override
    JLabel titulo() {
        return new JLabel("EDITAR QUESTÃO\nMULTIPLA ESCOLHA COM VARIAS RESPOSTAS");
    }

    protected void limpa() {
        respostaQuestao=null;
        alternativasQuestao=null;
        pergunta.setText("");
        boxResposta.removeAll();
